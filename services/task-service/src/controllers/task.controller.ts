...
import { channel, CHANNEL_NAME } from '../lib/rabbitmq';
...
export const createTask = async (req: Request, res: Response) => {
  try {
    ...
    const message = {
      event: 'task_created',
      data: { id: newTask._id, title: newTask.title },
    };

    if (channel) {
      await channel.sendToQueue(CHANNEL_NAME, Buffer.from(JSON.stringify(message)), {
        persistent: true,
      });
    } else {
      console.warn('RabbitMQ channel is not available. Task creation event will not be published.');
    }
